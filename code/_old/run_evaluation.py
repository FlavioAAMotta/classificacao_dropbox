...
def process_opt():
    ...
    parser.add_option("-c", "--classifier", dest="CLASSIFIER", default=0, type=int, help="1-SVM 2-RF (default 0-ONLINE)")
...
def Acerto_Erros(df,window):
    ...
    print("Perform", window,df.shape[0], accuracy_score(df['label'], df['pred']),
                        roc_auc_score(df['label'], df['pred']),
                        f1_score(df['label'], df['pred']),
                        auc(recall1,precision1),
    ...
    for window in range(win_start, opt.WINDOWS+1):
        ...
        if opt.CLASSIFIER:
            ...
            if window == win_train:
                ...
                df_train = arqAcessos.loc[name_space[window-1]].iloc[:,ini_train:fim_train]
                df_train.columns = ['acc1','acc2','acc3','acc4']
                '''
                #TEST: various features
                df_train = pd.concat([
                    feature_acc.loc[name_space[window-1]].iloc[:, ini_train:fim_train],
                    feature_log.loc[name_space[window-1]].iloc[:, ini_train:fim_train],
                    feature_obj.loc[name_space[window-1]].iloc[:, ini_train:fim_train],
                    feature_ses.loc[name_space[window-1]].iloc[:, ini_train:fim_train],
                    df_vol_norm.loc[name_space[window-1]].iloc[:, ini_train:fim_train]], axis=1, ignore_index=True)
                df_train.columns = [
                    'acc1','acc2','acc3','acc4',
                    'log1','log2','log3','log4',
                    'obj1','obj2','obj3','obj4',
                ...
                #   keep namespaces (rows) and go to the next time window (cols)
                ...
                if opt.SWEIGHT: # vol is the weight
                    clf.fit(df_train, df_class.values.ravel(), sample_weight=df_train.iloc[:,-1].values)
                else:
                ...
                if opt.ML == RF:
                    ...
                    f = plt.figure()
                    plt.barh(df_train.columns[sorted_idx], clf.feature_importances_[sorted_idx])
                    plt.xlim(0,0.20)
                    plt.xlabel("Random Forest Feature Importance")
                    f.savefig("RF_feat_imp_w" + str(window) + "_" + opt.LOG_ID + "allFeatures.pdf", bbox_inches='tight')
                '''
            ...
            ##TEST: various features        
            df_test = pd.concat([
                feature_acc.loc[name_space[window]].iloc[:, ini_test:fim_test],
                feature_log.loc[name_space[window]].iloc[:, ini_test:fim_test],
                feature_obj.loc[name_space[window]].iloc[:, ini_test:fim_test],
                feature_ses.loc[name_space[window]].iloc[:, ini_test:fim_test],
                df_vol_norm.loc[name_space[window]].iloc[:, ini_test:fim_test]], axis=1, ignore_index=True)
            '''
