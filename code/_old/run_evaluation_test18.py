...
if __name__ == '__main__':
    ...
    #remove the top 5 high vol objects seen at each window 
    # goal: to remove neg gain in undersampling_vol_weight
    s = set()
    for i in range(arqVolBytes.shape[1]): 
        s.update( list(arqVolBytes.iloc[:,i].sort_values(ascending=False).head(5).index) )
    l = list(s)
    print( "Removed objetcts:", len(l), l )
    arqAcessos = arqAcessos.drop(l)
    arqClasses = arqClasses.drop(l)
    arqVolBytes = arqVolBytes.drop(l)
