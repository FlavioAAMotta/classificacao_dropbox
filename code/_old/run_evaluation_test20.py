...
if __name__ == '__main__':
    ...
    df_train_cum = pd.DataFrame(columns = ['acc1','acc2','acc3','acc4','vol'])
    df_class_cum = pd.DataFrame(columns = [0], dtype='int64')
    ...
    for window in range(win_start, opt.WINDOWS+1):
        ...
        if opt.CLASSIFIER:
            ...
            #remove intances with zero acc
            df_acc_sum = pd.Series( index=df_acc_train.index,
                data=[reduce((lambda x,y: x + y), [int(val) for val in accs]) for namespace, accs in df_acc_train.iterrows()])
            idx = df_acc_sum > 0
            ...
            df_train_cum = pd.concat( [df_train_cum, df_train] )
            df_class_cum = pd.concat( [df_class_cum, df_class] )
                                    
            # train the classifier
            balancer = RandomUnderSampler()
            df_train, df_class = balancer.fit_sample(df_train_cum, df_class_cum)
            df_sweight = (np.log(df_train_cum.vol/min(df_train_cum.vol))/np.log(100))+1 # vol are weights of instances for training
        ...
        #remove instances with zero acc in current window
        idx = df.loc[:,'acc_cur'] > 0.0
        df = df.loc[idx,:]        
